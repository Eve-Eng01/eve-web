...
import EventFormatSelector from "../../../accessories/event-format-selector";
    ...
    <>
      <div className="bg-[#F4F4F4] p-[20px]">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 mt-2">
          ...
          <div className="grid grid-cols-7 gap-[30px]">{renderCalendar()}</div>
        </div>
...
const CreateEvent = ({ onNavigateToTab }: CreateEventProps) => {
  ...
  const [customUrl, setCustomUrl] = useState(
    "https://events.example.com/wedding/Anthony-Mary"
  );
  ...
  const handleNext = () => {
    if (currentStep === 0) {
      ...
      if (
        ...
        dateTimeEntries.every((entry) => !entry.startTime || !entry.endTime)
      ) {
        alert(
          "Please select at least one date with start and end times for a recurring event."
        );
        return;
      }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="text-gray-600 text-center max-w-md mb-8">
          You don't have any events happening right now. Current events will
          appear here.
        </p>
