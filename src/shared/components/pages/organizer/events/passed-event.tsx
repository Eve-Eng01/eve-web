...
type PassedEventProps = {
  isActive?: boolean;
};

const PassedEvent = ({ isActive = false }: PassedEventProps) => {
  ...
  const formattedEvents = useMemo(() => {
    return completedEvents.map((event: Event) => {
    ...
    return (
      <div className="w-full">
        <div className="flex flex-col items-center justify-center py-12">
          <p className="text-red-600 mb-4">Failed to load past events</p>
          <button
  ...
  if (formattedEvents.length === 0) {
    return (
      <EmptyEventCard
        ...
        description="Your completed events will be archived here for future reference and insights."
