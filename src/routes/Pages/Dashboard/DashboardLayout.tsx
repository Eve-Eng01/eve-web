...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children, user }) => {
  ...
  const vendorNavItems: NavItem[] = [
    { icon: Users, label: 'Browse Vendors', href: '/vendors', isActive: activeSection === 'Browse Vendors' },
    { icon: FileText, label: 'RFPs & Proposals', href: '/proposals', isActive: activeSection === 'RFPs & Proposals' },
    { icon: MessageSquare, label: 'Messages', href: '/messages', isActive: activeSection === 'Messages' },
  ];

  const bottomNavItems: NavItem[] = [
    { icon: Settings, label: 'Setting', href: '/settings', isActive: activeSection === 'Setting' },
    { icon: User, label: 'Account', href: '/account', isActive: activeSection === 'Account' },
          ...
          >
            <item.icon 
              className={`mr-3 h-5 w-5 ${
                item.isActive ? 'text-purple-500' : 'text-gray-400 group-hover:text-gray-500'
              }`} 
            />
            {item.label}
          </button>
        ...
        <div className="flex h-screen bg-gray-50">
            ...
            <div className="hidden md:flex md:w-64 md:flex-col">
                ...
                <div className="flex-1 px-3">
                    ...
                    <NavSection title="VENDORS & PROPOSALS" items={vendorNavItems} />
                </div>

                {/* Bottom Navigation */}
                <div className="px-3 mt-auto">
                    <NavSection title="GENERAL" items={bottomNavItems} />
                </div>
                </div>
