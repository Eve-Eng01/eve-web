...
import SalesRevenueChart from '../../../Accessories/SalesRevenueChart';
...
const PayoutsTab = () => {
  ...
  const [selectedPeriod, setSelectedPeriod] = useState<'24 hours' | '7 days' | '30 days'>('7 days');
  ...
  const generateMockPayouts = (): Payout[] => {
    ...
    const statuses: ('Successful' | 'Pending' | 'Failed')[] = [
      'Successful', 'Successful', 'Successful', 'Pending', 'Pending', 'Failed'
    ];
    
    // Based on Figma: amounts range from $22,000 to $90,000
    const amounts = [90000, 80000, 76000, 66000, 56000, 34000, 22000];
    const transferFees = [10.00, 24.00, 24.90, 50.00, 50.99, 74.00];
  ...
  const StatusBadge = ({ status }: { status: 'Successful' | 'Pending' | 'Failed' }) => {
    const statusConfig = {
